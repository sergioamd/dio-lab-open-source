...
2. Clonar localmente: `git clone https://github.com/YOURUSERNAME/dio-lab-open-source.git`;
3. Agregue el upstream remoto para mantener actualizado su repositorio local: `git remoto add upstream https://github.com/elidianaandrade/dio-lab-open-source.git`;
    > Utilice el comando `git pull upstream main` para desplegar y fusionar cambios en su repositorio local según la rama `main` de este repositorio original del que se bifurcó, o `git fetch upstream main` para desplegar sin fusionar. Ver más en: [Primeros pasos con Git y GitHub](https://github.com/elidianaandrade/dio-curso-git-github/blob/main/materiais-de-apoio/03-primeiros-passos-com-git -e-github.md).
4. Cree una nueva **rama** y asígnele el nombre `feat/community/yourusername`: `git checkout -b feat/community/yourusername`;
    > Ejemplo: `git checkout -b hazaña/comunidad/elidianaandrade`
5. Dentro de la carpeta [`community`](https://github.com/elidianaandrade/dio-lab-open-source/tree/main/community), cree un archivo en Markdown (extensión `.md`) y nombre con el mismo nombre que su nombre de usuario de GitHub;
...
8. Cree una confirmación y agregue el mensaje que indica la adición de su perfil `git commit -m"feat: add yourusername perfil"`;
9. Envíe los cambios a su repositorio remoto `git push origin feat/community/yourusername`;
10. Cree una **Solicitud de extracción**.
